...
def preprocess_resume(resume, resume_edu, resume_cert, resume_lang):
    ...
    # 시간 관련 정수 데이터 만들기
    resume['reg_year'] = resume['reg_date'].dt.year
    resume['reg_month'] = resume['reg_date'].dt.month
    resume['reg_day'] = resume['reg_date'].dt.day

    resume['updated_year'] = resume['updated_date'].dt.year
    resume['updated_month'] = resume['updated_date'].dt.month
    resume['updated_day'] = resume['updated_date'].dt.day
...
def preprocess_for_cosine(directory):
    # 데이터 로드
    file_names = ['resume', 'resume_education', 'resume_certificate', 'resume_language', 'recruitment', 'company']
    ...
    # 데이터 원-핫 인코딩, 스케일링
    resume_for_cosine = get_modified_data_except_field(resume, all_fields, cont_fields, cat_fields)
    recruitment_for_cosine = get_modified_data_except_field(recruitment, all_fields, cont_fields, cat_fields)

    return resume_for_cosine, recruitment_for_cosine
...
if __name__ == '__main__':
    ...
    resume_for_cosine, recruitment_for_cosine = preprocess_for_cosine(args.directory)

    resume_for_cosine.to_csv(f'{args.directory}/resume_for_cosine.csv', index=False)
    recruitment_for_cosine.to_csv(f'{args.directory}/recruitment_for_cosine.csv', index=False)
